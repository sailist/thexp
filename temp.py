...
from thexp.globals import PLUGIN_DIRNAME_
...
def _board_with_logdir(logdir, tmpdir, *args, **kwargs):
    ...
    subprocess.check_call(
        ['tensorboard', '--logdir={}'.format(logdir)] + ["--{}={}".format(k, v) for k, v in kwargs.items()],
        env=dict(os.environ))
...
def board(*args, **kwargs):
    ...
    if 'logdir' in kwargs:
        tmpdir = os.path.join(os.path.dirname(kwargs['logdir']), PLUGIN_DIRNAME_.writer_tmp)
        ...
        _board_with_logdir('./board', PLUGIN_DIRNAME_.writer_tmp, **kwargs)
