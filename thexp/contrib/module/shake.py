...
class ShakeShake(contrib.autograd.Function):

    @staticmethod
    def forward(ctx, x1, x2, training=True):
        if training:
            alpha = torch.zeros(x1.size(0)).uniform_().to(x1.device)
    ...
    def backward(ctx, grad_output):
        beta = torch.zeros(grad_output.size(0)).uniform_().to(grad_output.device)
...
class ShakeResNet(nn.Module):
    ...
    def __init__(self, depth=28, w_base=64, label=10):
        """
...
if __name__ == '__main__':
    model = ShakeResNet(28, 32, 10) # type:nn.Module
